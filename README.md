...
The features added in v1.1.0 extend this idea further. Channels, Scheduled Tasks, Remote Control, and Auto Memory don't change what Claude Code is — they change when and where it works. Claude Code is no longer only a tool you invoke. It can watch for events, run on a schedule, stay reachable from your phone, and learn from your corrections over time. The configuration layer got bigger. The principle stayed the same.
